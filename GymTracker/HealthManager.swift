...
class HealthManager: NSObject, ObservableObject {
        ...
        return await withCheckedContinuation { continuation in
            let query = HKSampleQuery(sampleType: sleepType, predicate: predicate, limit: HKObjectQueryNoLimit, sortDescriptors: nil) { _, samples, error in
                ...
                for (day, daySamples) in groupedSamples {
                    ...
                    // Use the robust calculation logic we fixed earlier
                    dailyTotals[day] = self.calculateDailyDuration(forSegments: data)
                }
    ...
    private func calculateDailyDuration(forSegments segments: [SleepData]) -> TimeInterval {
    ...
    private func calculateDailiyDuration(for data: [SleepData]) -> TimeInterval {
        return calculateDailyDuration(forSegments: data)
    }
}
