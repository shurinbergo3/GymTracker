...
*   **Умный Трекинг**:
    ...
    *   Недельная цель по тренировкам, стрики и календарь.
    *   Шеринг тренировки красивой карточкой (с данными прошлой сессии).
*   **AI-Коуч** 🤖:
    *   Диалоговый тренер на базе **OpenAI `gpt-5-mini`**.
    *   Контекст (последние сессии, замеры, прогресс) собирается автоматически — копипастить ничего не нужно.
    *   Запросы идут через **Vercel-прокси**, подписанный Firebase ID-токеном: ключ OpenAI не зашит в приложение, а модель выбирается на стороне сервера (смена модели — без обновления App Store).
    *   История диалогов синхронизируется в Firestore.
...
*   **Облачная Синхронизация (Firebase)** ☁️:
    ...
    *   Вход через Apple, Google или Email — со сбросом пароля.
*   **Локализация** 🌍:
    *   4 языка: 🇷🇺 Русский · 🇬🇧 English · 🇵🇱 Polski · 🇩🇪 Deutsch.
    *   Язык переключается внутри приложения (`LanguageManager`), независимо от системной локали — строки берутся через `.localized()`.
...
*   **AI**: **OpenAI `gpt-5-mini`** через Vercel-прокси (`GroqClient.swift` → `/api/ai-coach`); запрос подписывается Firebase ID-токеном.
*   **Локализация**: String Catalog + кастомный `LanguageManager` (`.localized()`), 4 языка (ru/en/pl/de).
...
│   │   ├── AICoach/                  # OpenAI-прокси клиент + сборка контекста
...
*   **AI Coach** (`Services/AICoach/`):
    *   `GroqClient.swift`: actor-клиент к Vercel-прокси `/api/ai-coach`; модель — OpenAI `gpt-5-mini`, ключ только на сервере, запрос подписан Firebase ID-токеном.
    *   `AICoachContextBuilder.swift`: сборка контекста (последние тренировки, замеры, прогресс) под промпт.
    *   `AICoachStore.swift` / `AICoachFirestore.swift`: хранение и синхронизация истории диалогов.
