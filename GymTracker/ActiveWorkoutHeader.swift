...
struct ActiveWorkoutHeader: View {
    ...
    private var progressState: ProgressState {
        // Simple logic: Compare total completed sets volume vs expected or previous
        // For now, let's use a placeholder logic or derived from `countImprovements`
        //Ideally this would be a real calculation of Volume vs Previous Session Volume
        return .improved // Defaulting to positive for the UI demo as requested ("with arrows")
    }
...
fileprivate struct HeaderBentoCard<Content: View>: View {
    let color: Color
    let content: Content
